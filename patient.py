...
def print_patient_table(patient_list):
    ...
    xyz=table.draw()
    TABLE=tk.Label(root,text=xyz).place(x=0,y=0)
...
def draw_gantt_chart(patient_list):
    ...
    # colors for priority
    priority_colors = {}  # Dictionary to store colors for each priority
    
    priority_lengths = {}  # Dictionary to store lengths for each priority


    # Loop through patients and assign colors based on priority
    for patient in patient_list:
        priority = patient.priority
        if priority not in priority_colors:
            # Generate a random color for the priority if not assigned yet
            priority_colors[priority] = f"rgb({random.randint(0, 255)}, {random.randint(0, 255)}, {random.randint(0, 255)})"
            priority_lengths[priority] = 1
        else:
            priority_lengths[priority] += 1
    ...
    for patient in patient_list:
        for i in range(len(patient.start_times)):
            data_frame_list.append(dict(
                Patient=str(patient.patient_id), 
                Start=seconds_to_timestamp(patient.start_times[i]),
                Finish=seconds_to_timestamp(patient.end_times[i]),
                Priority=str(patient.priority),
                Color=priority_colors[patient.priority],
                Length=priority_lengths[patient.priority]
                ))
    ...
    fig = px.timeline(df, x_start="Start", x_end="Finish",
                      y="Patient", color="Priority")
    # fig.update_yaxes(autorange="reversed")
    # Save the graph as an ihtml file
...
##    fig.write_html("D:/MMC PRIORITY tkinter/sem 6/output_graph.html")
